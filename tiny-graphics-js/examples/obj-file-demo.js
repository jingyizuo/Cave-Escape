...
export class Obj_File_Demo extends Scene     
  {                           // **Obj_File_Demo** show how to load a single 3D model from an OBJ file.
    ...
    constructor()                               
      { super();
                                      // Load the model file:
        this.shapes = { "teapot": new Shape_From_File( "assets/torch2.obj" ) };

                                      // Don't create any DOM elements to control this scene:
        this.widget_options = { make_controls: false };
                                                          // Non bump mapped:
      //  this.stars = new Material( new defs.Textured_Phong( 1 ),  { color: color( .5,.5,.5,1 ),
       //   ambient: .3, diffusivity: .5, specularity: .5, texture: new Texture( "assets/copper.png" ) });
                                                           // Bump mapped:
    ...
    display( context, program_state )
      { const t = program_state.animation_time;
        ...
        const key_transform = Mat4.identity();
        const model_transform = Mat4.translation(-1,0,0);
 ...
 export class key extends Scene
 {
 }